...
def plot_gradients(
    ...
    xx = np.stack(
        np.meshgrid(np.linspace(*xlim, nx), np.linspace(*ylim, ny)), axis=-1
    ).reshape(-1, 2)
    ...
    if plot_scatter:
        ...
        plt.scatter(
            data[:, 0], data[:, 1], alpha=0.3, color="red", edgecolor="white", s=40
        )
        plt.xlim(-1.5, 2.0)
        plt.ylim(-1.5, 2.0)

    quiver = plt.quiver(*xx.T, *scores_log1p.T, width=width, color="white", alpha=alpha)
...
def make_animation(
    ...
    xlim_scale: float = 1.0,
    ylim_scale: float = 1.0,
):
    ...
    xlim = (Xt[..., 0].min() * xlim_scale, Xt[..., 0].max() * xlim_scale)
    ylim = (Xt[..., 1].min() * ylim_scale, Xt[..., 1].max() * ylim_scale)

    ax.axis("off")
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)

    if model is not None and data is not None and forward is not None:
        quiver = plot_gradients(
            model,
            data,
            forward,
            xlim=xlim,
    ...
    scatter = ax.scatter(Xt[0, :, 0], Xt[0, :, 1], s=10)

    def animate(i):
        scatter.set_offsets(Xt[i])
        return quiver_tuple + (scatter,)

    anim = animation.FuncAnimation(
        fig,
        animate,
        init_func=lambda: animate(0),
        frames=np.linspace(0, len(Xt) - 1, len(Xt) // step_size, dtype=int),
        interval=interval,
