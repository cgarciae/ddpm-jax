...
class ConditionalModel(tx.Module):
    def __init__(self, timesteps: int):
        ...
        self.lin1 = ConditionalLinear(28 * 28, timesteps)
        self.lin2 = ConditionalLinear(28 * 28, timesteps)
        self.lin3 = ConditionalLinear(28 * 28, timesteps)
        self.lin4 = tx.Linear(28 * 28)
...
def plot_trajectory(
    ...
    for plot_i, (i, t) in enumerate(zip(idxs, ts)):
        cur_x = x_seq[i]
        axs[plot_i].imshow(cur_x.reshape(28, 28), cmap="gray")
...
def main(
    ...
    for i in tqdm(range(steps + 1)):
        ...
        if i % plot_steps == 0:
            ...
            if viz:
                x_seq = sampler.reverse_sample_loop(model, (1, 28 * 28))
                print("Plotting...")

                # print(x_seq[-1])

                plot_trajectory(x_seq[::-1][:, 0], n_plots=5, reverse=True)
